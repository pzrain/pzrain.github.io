...
* **记号与迭代方程**
  ...
  \end{align}
  $$
  $$d=1,2,\cdots,D$$。其中$$w$$为**惯性权重**，$$c_1$$与$$c_2$$为**加速常数**，$$\mathtt{rand}()\in[0,1]$$为随机数。此外，更新过程中需要保持$$|v_{id}|\leq v_{max,d}$$。$$v_{max}$$决定了在当前位置与最好位置之间的区域的分辨率，如果$$v_{max}$$太高，微粒可能会飞过最好解；而如果$$v_{max}$$太小，微粒就不能进行足够的探索。此外，为了防止发散，$$w$$必须比1小。通常$$c_1$$与$$c_2$$的取值范围为$$[1,3]$$。
